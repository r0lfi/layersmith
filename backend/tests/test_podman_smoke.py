"""End-to-end build against real Podman.

Skipped unless LAYERSMITH_SMOKE=1 and podman is on PATH, because it pulls a
base image from the network and takes a minute. Everything else in the suite
runs against a fake backend.
...
pytestmark = pytest.mark.skipif(
    not (os.environ.get("LAYERSMITH_SMOKE") and shutil.which("podman")),
    reason="needs podman and LAYERSMITH_SMOKE=1",
)
...
def service(tmp_path):
    ...
    subprocess.run([settings.podman_binary, "rmi", "-f", f"{IMAGE}:{VERSION}"], capture_output=True)


def test_podman_is_usable(service):
...
def test_real_build_export_and_airgap_bundle(service, tmp_path):
    ...
    for command, expected in [
        ...
        done = subprocess.run(["podman", "run", "--rm", reference, *command],
    ...
    subprocess.run(["podman", "rmi", "-f", reference], capture_output=True)
    loaded = subprocess.run(["podman", "load", "-i", export_path], capture_output=True, text=True, timeout=300)
